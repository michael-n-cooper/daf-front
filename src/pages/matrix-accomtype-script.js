...
Promise.all(promises).then((values) => {
	let innerPromises = new Array();

	innerPromises.push(apiGet("functional-abilities"));
	...
	let accessibilityCharacteristicGroups = new Array();
	let functionalAbilityGroups = new Array();

	gpChar.forEach(async function(group) {
		innerPromises.push(async function() {
			let val = await apiGet("accessibility-characteristic-groups/" + idFrag(group.id));
			accessibilityCharacteristicsGroups.push(val);
		});
	});
	gpFa.forEach(async function(group) {
		innerPromises.push(async function() {
			let val = await apiGet("functional-ability-groups/" + idFrag(group.id));
			functionalAbilityGroups.push(val);
		});
	});
	...
	Promise.all(innerPromises).then(() => {
		...
		accessibilityCharacteristicGroups.forEach(function (group) {
			...
			link.href = base + "accessibility-characteristic-groups/" + idFrag(group.id);
		...
		accessibilityCharacteristicGroups.forEach(function (charGroup) {
			...
			members.forEach(function (item) {
				...
				let link = document.createElement("a");
				link.href = base + "accessibility-characteristics/" + idFrag(item.id);
		...
		functionalAbilityGroups.forEach(function (faGroup) {
			...
			fagLink.href = base + "functional-ability-groups/" + idFrag(faGroup.id);
			...
			faMembers.forEach(function (functionalAbility) {
				let faRow = faCount == 0 ? fagRow : document.createElement("tr");
				faCount++;
				
				counts[functionalAbilityId] = 0;

				//col 2 header
				let functionalAbilityId = idFrag(faGroup.id) + "+" + idFrag(functionalAbility.id);
				...
				faLink.href = base + "functional-abilities/" + idFrag(functionalAbility.id);
				faLink.append(document.createTextNode(functionalAbility.label));
				...
				accommodationTypes.forEach(function(accommodationType) {
					let accTypeRow = accTypeCount == 0 ? faRow : document.createElement("tr");
					accTypeCount++;
					...
					accTypeLink.href = base + "accommodation-types/" + idFrag(accommodationType.id);
					accTypeLink.append(document.createTextNode(accommodationType.label));

					accTypeCell.append(link);
					accTypeRow.append(accTypeCell);

					// accessibility characteristic groups and individuals
					accessibilityCharacteristicGroups.forEach(function (charGroup) {
						charGroup.members.forEach(function (characteristic) {
							let cell = document.createElement("td");

							let maps = filterObjectByProperties(simpleCurveMaps, { "abilityId": functionalAbility.id, "accomId": accommodationType.id, "charId": characteristic.id });
							...
							if (maps.length > 0) {
								...
								*/
								...
								maps.forEach(function (map) {
									if (map.stmtId != null) {
										let stmt = findObjectByProperties(statements, { "id": map.stmtId });
										...
										counts[idFrag(charGroup.id)]++;
										counts[functionalAbilityId]++;
										counts[idFrag(charGroup.id)]++;
										counts[idFrag(characteristic.id)]++;
									}
								});
